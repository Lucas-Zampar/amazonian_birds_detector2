...
def make_species_proportion_bar_plot(filepath, species_proportion_filename, species_list, is_english=False): 
    '''
    Adicionar docstring
    '''
    ...
    if not is_english:
        ...
        label_map = {
            'canario_do_amazonas': 'Orange-fronted Yellow-finch', 
            'chupim': 'shiny Cowbird', 
            'rolinha':'Ground Dove', 
            'sanhaco_da_amazonia':'Blue-gray Tanager', 
            'sanhaco_do_coqueiro':'Palm Tanager',
            'bem_te_vi': 'Great Kiskadee'
        }
    ...
    for species in species_list:
        ...
        fig.add_trace(
            go.Bar(
                ...
                hovertemplate = f'<b>{species}</b><br>'+'Anotações: %{y}<extra></extra>'
            )
        )

    fig.update_layout(
        height=1200,
        #title = "Total de anotações por espécie.",
    ...
    fig.update_xaxes(
        tickangle=45,
        tickfont = dict(size=30) # for paper
        #tickfont = dict(size=24)
    )

    fig.update_yaxes(
        #title = "Labels", # for paper 
        titlefont = dict(size=24),
        tickfont = dict(size=24)
    )  

    #fig.write_image(species_proportion_filename, format='png', width=1280, height=900)  
    fig.write_image(species_proportion_filename, format='png', width=1920, height=1080) # for paper
